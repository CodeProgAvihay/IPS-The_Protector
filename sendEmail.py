...
בברכה,  
מערכת ה־IPS'''
    elif type == "Dns Tunnle":
        body += f'''שלום,

מערכת ה־IPS זיהתה פעילות זדונית מסוג DNS Tunneling ברשת.

מתקפת DNS Tunneling היא מתקפה שבה תוקף מנצל את פרוטוקול ה־DNS
לצורך העברת מידע מוסתר בתוך בקשות ותגובות DNS.
שיטה זו מאפשרת עקיפת מנגנוני אבטחה, יצירת ערוץ תקשורת סמוי (Covert Channel),
זליגת מידע רגיש ואף שליטה מרחוק במערכת שנפרצה.

פרטי האירוע:
כתובת IP חשודה: {ip}
רמת חומרה: קריטי (Critical)

המערכת זיהתה תעבורת DNS אנומלית, הכוללת בקשות חריגות בתדירות גבוהה
ו/או תתי־דומיינים ארוכים בצורה בלתי רגילה, ונקטה בפעולות הגנה בהתאם להגדרות,
כולל חסימת התעבורה מהמקור החשוד והתראה למנהל המערכת.

המלצות:
לנתח את לוגי ה־DNS לאיתור דפוסי תעבורה חריגים
לוודא שהדומיין החשוד אינו שירות לגיטימי
לשקול הגבלת בקשות DNS חריגות באמצעות Firewall / DNS Filtering
להמשיך במעקב אחר ניסיונות תקשורת חשודים נוספים
